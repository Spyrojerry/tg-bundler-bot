...
export class InsiderBot extends EventEmitter {
  ...
  private async handleFollowInsiderMatchingFeeSellTx(
    ...
    if (tx.fee === undefined || this.followInsiderMatchingFeeLamports === null) return;
    const solPriceUsd = await this.getCachedSolPriceUsd();
    if (solPriceUsd === null) return;
    const feeDifferenceUsd =
      (Math.abs(tx.fee - this.followInsiderMatchingFeeLamports) * solPriceUsd) /
      1_000_000_000;
    ...
    for (const entry of tx.accountData ?? []) {
      walletAddresses.add(entry.account);
    }
    if ([...walletAddresses].some((wallet) => this.followInsiderMatchingFeeExcludedWallets.has(wallet))) {
      return;
    }

    const hasTargetOut = (tx.tokenTransfers ?? []).some(
      (transfer) => transfer.mint === mint && transfer.fromUserAccount !== UNKNOWN_COUNTERPARTY,
    );
