...
interface FollowTokenEarlyBundlerExitState {
  ...
  /** Post-LI sold-all buy blocked telegram already sent (max single sell gate). */
...
export class InsiderBot extends EventEmitter {
  ...
  private async startInsiderFlowFromMigration(
    ...
    void this.sendTelegramSafe(
      [
        ...
        "GMGN bundler poll runs for logging only (no buy/tag-plan gates).",
  ...
  private async startFollowTokenGmgnOnlyFlow(
    ...
    if (!largeInsiderStarted) {
      await this.resetFollowTokenAfterLargeInsiderStartFailed(
        mint,
        "gmgn_flow_started",
        "large insider flow — GMGN flow start",
      );
      return;
    }
    ...
    void this.sendTelegramSafe(
      [
        ...
        "",
        `GMGN poll every ${FOLLOW_TOKEN_GMGN_BUNDLER_POLL_INTERVAL_MS / 1_000}s (logging only — no buy or flow gates).`,
    ...
    void this.sendTelegramSafe(
      [
        ...
        activeFunderWatch.lowFundingMode
          ...
          : this.flowSource === "follow-token"
            ? `Follow-token buy triggers: Large Insider on valid wallet <b>#${FOLLOW_TOKEN_LARGE_INSIDER_BUY_AT_VALID_WALLET_COUNT}</b> or early bundler sold-all (still watch for #5 · +${FOLLOW_TOKEN_LARGE_INSIDER_PROFIT_EXIT_PERCENT}% MC TP · any valid wallet ≥25% sell early exit). GMGN poll is logging-only. Reset on 20m window closes or no scrape watches before buy. Large Insider: feePayer ≥${FOLLOW_TOKEN_LARGE_INSIDER_MIN_FEEPAYER_OUT_SOL} SOL outs (${FOLLOW_TOKEN_LARGE_INSIDER_FEEPAYER_WINDOW_SEC / 60}m after initial bundler first buy) → downstream watches only on ≥${FOLLOW_TOKEN_LARGE_INSIDER_MIN_CHAIN_OUT_SOL} SOL outs (tier1→chain→…). Round/dust gates disabled.`
  ...
  private startFollowTokenGmgnBundlerPoll(state: BundlerFunderWatchState): void {
    ...
    this.followTokenGmgnBundlerBackend("GMGN bundler poll started (logging only)", {
      mint: state.mint,
      devCreateTimestamp,
      pollIntervalMs: FOLLOW_TOKEN_GMGN_BUNDLER_POLL_INTERVAL_MS,
  ...
  private async pollFollowTokenGmgnBundlerBuy(): Promise<void> {
    ...
    try {
      ...
      this.followTokenGmgnBundlerBackend(
        "GMGN second bundler group observed (logging only — no action)",
        {
          mint: state.mint,
          firstGroupTimestamp:
            this.followTokenGmgnInitialBundlerGroup.anchorTimestamp,
          secondGroupTimestamp: nextGroup.anchorTimestamp,
          secondGroupWalletCount: nextGroup.wallets.length,
          secondGroupWallets: nextGroup.wallets,
          groups: this.summarizeGmgnBundlerGroupsForLog(groups),
        },
      );
    } catch (err) {
  ...
  private notifyBundlerSoldAllMaxSingleSellBuyBlocked(
    ...
    const topWatch =
      this.getFollowTokenEarlyBundlerExitHighestCumulativeSellUsdWatch();
    const limit = FOLLOW_TOKEN_EARLY_BUNDLER_PRE_LI_MAX_SINGLE_SELL_TOKEN_AMOUNT;
    const topWatchLine = topWatch
      ? `Highest cumulative-USD watch <code>${topWatch.wallet}</code> has a single sell tx of <b>${topWatch.maxSingleSellTokenAmount.toLocaleString()}</b> tokens (limit <b>${limit.toLocaleString()}</b>).`
      : "";
    ...
    this.log.warn(
      "Post-LI bundler sold-all buy blocked — max single sell token amount exceeded on highest cumulative-USD watch",
      {
        mint: funderState.mint,
        topWallet: topWatch?.wallet ?? null,
        maxSingleSellTokenAmount: topWatch?.maxSingleSellTokenAmount ?? 0,
    ...
    void this.sendTelegramSafe(
      [
        ...
        topWatchLine,
        "",
        "Valid LI wallet #1+ found — bundler sold-all buy requires max single sell ≤ limit. Waiting for valid wallet #4 (direct-sell) or a later sold-all eval that passes the gate.",
      ]
        .filter(Boolean)
        .join("\n"),
  ...
  private async maybeTriggerFollowTokenLargeInsiderPreBuyFromBundlerPath(
    ...
    await this.emitFollowTokenLargeInsiderBuy(
      ...
      signature,
      tx,
      { triggerSource: "bundler_sold_all", bundlerExitBranch: buyBranch },
    );
  }
  ...
  private async startFollowTokenEarlyBundlerExitMonitoring(
    ...
    void this.sendTelegramSafe(
      [
        ...
        "",
        "Started at Follow-Token GMGN flow. Synced from each initial buy tx (paginated). Token transfers chain **every** recipient (multi-recipient txs included); sender dropped only after full transfer-out. Recipient sync drains until no new watches remain.",
        this.buySubmitted
          ...
          : this.hasFollowTokenLargeInsiderValidWalletDiscovered()
            ? `Post–1st-LI buy: valid wallet #4 (direct-sell) OR sold-all branches (skip / MC TP / LI-only) when highest-USD watch max single sell ≤ ${FOLLOW_TOKEN_EARLY_BUNDLER_PRE_LI_MAX_SINGLE_SELL_TOKEN_AMOUNT.toLocaleString()} tokens. Transfer-out → sold-all only.`
        ...
        `After buy + all sold all (post–1st-LI): valid LI ≥25% → sell; max sell txs < ${FOLLOW_TOKEN_EARLY_BUNDLER_EXIT_MIN_SELL_TX_COUNT_FOR_USD_GATE} or cumulative ≤ $${FOLLOW_TOKEN_EARLY_BUNDLER_EXIT_LOW_SELL_USD_THRESHOLD.toLocaleString()} → skip unless max single sell ≤ ${FOLLOW_TOKEN_EARLY_BUNDLER_PRE_LI_MAX_SINGLE_SELL_TOKEN_AMOUNT.toLocaleString()} tokens (+${FOLLOW_TOKEN_LARGE_INSIDER_PROFIT_EXIT_PERCENT}% MC TP buy); ≥ ${FOLLOW_TOKEN_EARLY_BUNDLER_EXIT_MIN_SELL_TX_COUNT_FOR_USD_GATE} txs + $${FOLLOW_TOKEN_EARLY_BUNDLER_EXIT_LOW_SELL_USD_THRESHOLD.toLocaleString()}–$${FOLLOW_TOKEN_EARLY_BUNDLER_EXIT_HIGH_SELL_USD_MC_TP_DISABLE.toLocaleString()} → dual exit; cumulative > $${FOLLOW_TOKEN_EARLY_BUNDLER_EXIT_HIGH_SELL_USD_MC_TP_DISABLE.toLocaleString()} → LI-only.`,
      ].join("\n"),
